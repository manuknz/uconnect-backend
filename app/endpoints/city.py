...
def get_city_by_id(city_id: int, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
...
def get_city_by_name(city_name: str, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
...
def add_city(city: schemas.CityCreate, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
...
def edit_city(city_id: int, city: schemas.CityCreate, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
    db_city = services.get_city_by_id(db, city_id)
    if (db_city):
...
def delete_city(city_id: int, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
    db_city = services.get_city_by_id(db, city_id)
    if (db_city):
        db.delete(db_city)
        db.flush()
