...
def create_company(company: schemas.CompanyCreate, db: Session = Depends(get_db)):
    try:
        ...
        if (db_company):
            ...
            raise HTTPException(
                status_code=400, detail=ErrorMessage.BUSINESS_INFO_USER_EMAIL_REGISTERED.value)
...
async def recover_password(data: pass_schemas.PasswordEmail, db: Session = Depends(get_db)):
    try:
        ...
        return await email_services.send_password_recovery_email(name, email, password_reset_code)
    except HTTPException as ex:
        db.rollback()
