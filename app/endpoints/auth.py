...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    authenticate = auth_services.authenticate_user(db, form_data.username, form_data.password)
    ...
    if authenticate.get('user'):
        ...
        access_token_expires = timedelta(minutes=hash_variables.ACCESS_TOKEN_EXPIRE_MINUTES)
        ...
        access_token_expires = timedelta(minutes=hash_variables.ACCESS_TOKEN_EXPIRE_MINUTES)
        access_token = auth_services.create_access_token(data={"sub": company.email},
                                                         expires_delta=access_token_expires)
        return {'company': company, 'access_token': access_token, 'token_type': 'bearer'}
