...
def get_company_by_email(db: Session, email: str):
    return db.query(models.Company).filter(func.lower(models.Company.email) == email.lower()).first()
...
def create_company(db: Session, company: schemas.CompanyCreate):
    ...
    db_company = models.Company(email=company.email.lower(), password=hashed_password,
                                name=company.name)
